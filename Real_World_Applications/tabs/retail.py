...
class RetailTab:
    ...
    def Personalized_Product_Recommendations(self):
        ...
        <div style='
            ...
            text-align: center;
        '>
            <h4 style='color: #1a1a1a;'> 🔎 Filter Recommendations </h4>
        """,
        unsafe_allow_html=True
                )
        ...
        for i, rec in enumerate(recommended):
            ...
            with cols[i % 3]:
                ...
                st.write(f"⭐ {product['Rating']} ({product['ReviewCount']} reviews)")
                st.caption(f"Similarity: {score:.2f} | Tags matched: {overlap}")
                if st.button(f"🛒 Buy Now ({product['ProductID']})", key=f"buy_{product['ProductID']}"):
                    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    clicked_items.append(product['ProductID'])
                    st.success(f"🛍️ Redirecting to checkout for {product['ProductName']}...")
        ...
        if clicked_items:
           st.info(f"📝 Click Log Updated: {', '.join(clicked_items)}")

        # Then show your next component:
        selected_module = st.radio(
            "Select Finance Module",
            ["🚨 Credit Card Fraud Detection", "🏦 Loan Approval Prediction"]
        )
    ...
    def Dynamic_Pricing(self):
        ...
        <div style='
            ...
            text-align: left;
        '>
            <h4 style='color: #1a1a1a;'> 💸 Dynamic Pricing Simulator </h4>
        """,
        unsafe_allow_html=True
                )
        ...
        st.subheader("💰 Final Price Suggestion")
        ...
        - Adjusted from base price of ${base_price} → **${new_price}**
        """)
    ...
    def Visual_AI(self):
        ...
        <div style='
            ...
            text-align: center;
        '>
            <h4 style='color: #1a1a1a;'> 🧍‍♂️ AI Virtual Try-On Simulator </h4>
        </div>
        """, unsafe_allow_html=True)

        uploaded_file = st.file_uploader("Upload your selfie (front-facing)", type=["jpg", "jpeg", "png"], key="Photo1")
        selected_items = st.multiselect("Try on: 👓 🧢 👕", ["Glasses", "Hat", "T-Shirt"], key="Accesories1")
        ...
        def get_overlay_position(image, landmarks, item):
            ...
            if item == "Glasses":
                ...
                x_offset = 12 
                y_offset = 13  # 👈 Move hat 10 pixels higher (adjust as needed)
        ...
        if uploaded_file:
            ...
            st.success("✨ Virtual try-on applied using face detection.")
        else:
            st.info("📸 Please upload a front-facing photo to try virtual items.")
        ...
        <div style='
            ...
            text-align: center;
        '>
            <h4 style='color: #1a1a1a;'> 🧍‍♂️ AI Virtual Try-On Simulator 2.0 </h4>
        </div>
        """, unsafe_allow_html=True)
        ...
        def load_overlay(path):
            try:
                ...
                st.error(f"❌ Error loading overlay: {e}")
                return None
        ...
        if uploaded_file:
            ...
            selected_items = st.multiselect("Try on: 👓 🧢 👕", ["Glasses", "Hat", "T-Shirt"], key="Accesories2")
            ...
            st.image(result_img, caption="🧍 Result with Accessories", use_container_width=True)

            # 📤 Download button
            buf = BytesIO()
            result_img.save(buf, format="PNG")
            st.download_button("📥 Download Result", data=buf.getvalue(), file_name="tryon_result.png", mime="image/png")
        else:
            st.info("📸 Upload a front-facing selfie to begin.")
        ...
        st.sidebar.markdown("## 🎥 Webcam Try-On")
